        ...
        <li id="comment-<?php echo esc_html($comment->comment_ID); ?>" class="single-comment-area">
            ...
            <div class="comment-content">
                <div class="author-name-deg">
                    ...
                    <span><?php echo get_comment_date('m F, Y') ?> &nbsp; &nbsp; <?php echo get_comment_time() ?></span>
                </div>
                <?php comment_text() ?>
                <div>
                    <?php if ($depth < $args['max_depth'] && comments_open()) :  ?>
                        <div class="replay-btn">
                            <?php comment_reply_link(array_merge($args, array('depth' => $depth, 'max_depth' => $args['max_depth'], 'reply_text' => '<svg xmlns="http://www.w3.org/2000/svg" width="14" height="11" viewBox="0 0 14 11">
...
function wc_comment_form_change_cookies($fields)
{
    ...
    $fields['cookies'] = '<p class="comment-form-cookies-consent"><input id="wp-comment-cookies-consent" name="wp-comment-cookies-consent" type="checkbox" value="yes"' . $consent . ' />' .
        '<label for="wp-comment-cookies-consent">' . esc_html__('Save my name, email in this browser for the next time I comment.', 'drivco') . '</label></p>';
    return $fields;
}
